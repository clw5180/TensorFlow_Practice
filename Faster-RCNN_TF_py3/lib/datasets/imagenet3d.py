...
class imagenet3d(datasets.imdb):
    ...
    def gt_roidb(self):
        ...
        if cfg.IS_RPN:
            ...
            for i in range(1, self.num_classes):
                print('{}: Total number of boxes {:d}'.format(self.classes[i], self._num_boxes_all[i])) # clw modify: for py3
    ...
    def region_proposal_roidb(self):
        ...
        if os.path.exists(cache_file):
            with open(cache_file, 'rb') as fid:
                roidb = cPickle.load(fid)
            print('{} roidb loaded from {}'.format(self.name, cache_file)) # clw modify: for py3
            return roidb

        if self._image_set != 'test':
            gt_roidb = self.gt_roidb()

            print('Loading region proposal network boxes...') # clw modify: for py3
        ...
        print('wrote roidb to {}'.format(cache_file)) # clw modify: for py3
