...
DATASET_NAME = 'steer' # clw modify
PIXEL_MEAN = [123.68, 116.779, 103.939]  # R, G, B. In tf, channel is RGB. In openCV, channel is BGR
#IMG_SHORT_SIDE_LEN = 600
#IMG_MAX_LENGTH = 1000
IMG_SHORT_SIDE_LEN = 1000 # clw modify
IMG_MAX_LENGTH = 1200 # clw modify
#CLASS_NUM = 20
CLASS_NUM = 1  # clw modify: for steer_dataset
...
BASE_ANCHOR_SIZE_LIST = [256]  # can be modified
                               # clw modify : 看代码发现上面这个LIST其实只取了第1个元素，因此这里不用写什么16, 32, 64, 128了。
ANCHOR_STRIDE = [16]  # can not be modified in most situations
#ANCHOR_SCALES = [0.5, 1., 2.0]  # [4, 8, 16, 32]
ANCHOR_SCALES = [0.1, 0.14, 0.17, 0.2, 0.23, 0.26, 0.29, 0.33, 0.4, 0.5] # clw modify：钢筋检测中，框的尺寸在30-300之间，多数集中在70到200之间
                                                                # 用kemans得到一批在70到200之间的数，如果BASE_ANCHOR_SIZE=256，换算得到scale：
                                                                # [0.29, 0.34, 0.39, 0.45, 0.52, 0.57, 0.66, 0.8]
                                                                # 另外原图是2000*2666，我要缩放到1000*1333，因此上面这个值还要除以2
                                                                # 首尾再手动填个0.1和0.5，一共10个数；
ANCHOR_RATIOS = [0.5, 1., 2.0]

# 如果原图缩放到1000*1000差了才4倍，故考虑RATIO再调小一点
                                  #             用之前kmeans聚类的结果，大概和0.3也比较接近
