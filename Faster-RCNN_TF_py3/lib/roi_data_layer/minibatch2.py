...
def get_minibatch(roidb, num_classes):
    ...
    if cfg.TRAIN.HAS_RPN:
        ...
        for im_i in range(num_images): # clw modify: for py3
...
def _sample_rois(roidb, fg_rois_per_image, rois_per_image, num_classes):
    ...
    for i in range(1, num_classes): # clw modify: for py3
...
def _vis_minibatch(im_blob, rois_blob, labels_blob, overlaps, sublabels_blob, view_targets_blob=None, view_inside_blob=None):
    ...
    for i in range(min(rois_blob.shape[0], 10)):
        ...
        plt.imshow(im)
        print('class: ', cls, ' subclass: ', subcls, ' overlap: ', overlaps[i]) # clw modify:for py3
