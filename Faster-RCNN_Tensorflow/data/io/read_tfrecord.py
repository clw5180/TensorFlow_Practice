...
def read_single_example_and_decode(filename_queue):
    ...
    features = tf.parse_single_example(  # clw note：用 tf.parse_single_example() 解析数据
...
def next_batch(dataset_name, batch_size, shortside_len, is_training):
    ...
    filename_tensorlist = tf.train.match_filenames_once(pattern) # clw note：获取符合正则表达式的文件列表；文件顺序不确定
                        # 这里获取存储训练数据的文件列表（数据已转为 TFRecord 格式的一个或多个tfrecord文件，见上面的_train*）

    filename_queue = tf.train.string_input_producer(filename_tensorlist) # clw note：创建输入文件队列，默认shuffle=True
    ...
    img_name_batch, img_batch, gtboxes_and_label_batch, num_obs_batch = \
        tf.train.batch(  # clw note：将数据组合成batch，也有人用tf.train.shuffle_batch()
                       [img_name, img, gtboxes_and_label, num_obs],
                       batch_size=batch_size,
                       capacity=1,
                       num_threads=1,
                       dynamic_pad=True)
    return img_name_batch, img_batch, gtboxes_and_label_batch, num_obs_batch  # clw note：将该batch内容return，用于训练
